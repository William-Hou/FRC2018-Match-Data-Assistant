import requests
import json

from handlers import *
from utils import *

TEAM = 2521
TEAM = "frc" + str(TEAM)

def average_teleop_scale_ownership(team_key):
    matches = match_request_handler(team_key)
    total = []
    for match in matches:
        if match_exists(matches,match["key"],team_key):
            # print(match["score_breakdown"][alliance_color(match["key"])]["teleopSwitchOwnershipSec"])
            total.append(match["score_breakdown"][alliance_color(matches,match["key"],team_key)]["teleopScaleOwnershipSec"])
    return round(sum(total)/len(total),2)
    
    
if __name__ == "__main__":    
    print(average_teleop_scale_ownership(TEAM))
    